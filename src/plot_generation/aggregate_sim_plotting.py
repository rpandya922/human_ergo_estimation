...
# RANDOM_COLOR = '#c68400'
RANDOM_COLOR = '#929591'
DATA_LOCATION = '../data/2dof_arm_weights_testing'
# TRUE_MEAN = np.array([0.5, -1, 0.5, -0.5, -0.1, 0.1, -0.2])
np.random.seed(0)
# TRUE_MEAN = np.random.uniform(size=7) * 2
# TRUE_MEAN -= 1
...
def plot_beliefs(all_particles, ground_truth, title=''):
    fig, axes = plt.subplots(nrows=4, ncols=3, subplot_kw={'aspect': 'equal'})
    axes = np.ndarray.flatten(np.array(axes))
    fig.suptitle(title)
    for i, particles in enumerate(all_particles):
        particles1 = particles[:,:2] / np.linalg.norm(particles[:,:2], axis=1).reshape(-1, 1)
        ground_truth = ground_truth[:2] / np.linalg.norm(ground_truth[:2])
        utils.plot_belief(axes[i], particles1, ground_truth)
        plt.pause(0.01)
...
def neg_log_likelihood(data, ground_truth):
    ...
    for (theta, feasible) in data:
        total += pe.prob_theta_given_lam_stable_set_weight_num(theta, TRUE_MEAN, ground_truth, cost, 1)
        total -= pe.prob_theta_given_lam_stable_set_weight_denom(feasible, TRUE_MEAN, ground_truth, cost, 1)
    return -total
def calc_test_set_likelihood(all_particles, test_set, title=''):
    likelihoods = []
    for i, particles in enumerate(all_particles):
        weights = np.ones(particles.shape[0]) / particles.shape[0]
        dist = SetMeanParticleDistribution(np.array(particles), np.array(weights), \
        cost, TRUE_MEAN, 1, 1, 0.01)
        likelihoods.append(-dist.neg_log_likelihood_mean(test_set, i, title))
    return likelihoods
...
def likelihood_ground_truth_debugging(data, ground_truth):
    seaborn.set_style('ticks')
    ...
    for i, ax in enumerate(axes):
        ...
        def likelihood(idx, point):
            alpha = 1
            return pe.prob_theta_given_lam_stable_set_weight_num(point, TRUE_MEAN, ground_truth, cost, alpha)\
            -pe.prob_theta_given_lam_stable_set_weight_denom(feasible, TRUE_MEAN, ground_truth, cost, alpha)
        ...
        print pe.prob_theta_given_lam_stable_set_weight_num(theta, TRUE_MEAN, ground_truth, cost, 1)\
        -pe.prob_theta_given_lam_stable_set_weight_denom(feasible, TRUE_MEAN, ground_truth, cost, 1)
        plt.pause(0.01)
...
def get_test_set(ground_truth):
    ...
    for i in range(len(data)):
        feasible = data[i]
        probs = get_distribution(feasible, cost, ground_truth, ALPHA)
        test_set.append(create_sample(feasible, probs))
    return test_set
...
for set_idx in tqdm(range(20)):
    for param_idx in tqdm(range(5)):
        ...
        # test_set = get_test_set(weights)
        # likelihood_ground_truth_debugging(test_set, weights)
        ...
        # plot_beliefs(data['particles_active'], weights, 'active')
        # plot_beliefs(data['particles_passive'], weights, 'passive')
        # plot_beliefs(data['particles_random'], weights, 'random')
        ...
        # print "active"
        ll_active = calc_test_set_likelihood(data['particles_active'], test_set, 'active')
        # print "passive"
        ll_passive = calc_test_set_likelihood(data['particles_passive'], test_set, 'passive')
        # print "random"
        ll_random = calc_test_set_likelihood(data['particles_random'], test_set, 'random')
        ...
        data['expected_costs'], data['training_data'], weights)
        ...
        ground_truth_likelihood = np.array([-neg_log_likelihood(test_set, weights)] * 10)
