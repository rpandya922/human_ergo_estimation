...
def create_ellipse_rot(x0, y0, a, b, theta):
    rand = np.random.uniform(0, 1, size=(4000, 2))* np.array([12, 12])
    rand += np.array([-6, -6])
    x = rand[:,0]
    y = rand[:,1]
    ...
    rad_cc = (xct**2/(a/1.)**2) + (yct**2/(b/1.)**2)
    feas = []
...
np.random.seed(0)
centers = np.random.randint(-5, 5, size=(8, 2))
angles = np.random.randint(-90, 90, size=8)
data_original = []
for i, center in enumerate(centers):
    data_original.append(create_ellipse_rot(center[0], center[1], 1, 3, angles[i]))
